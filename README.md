...
* **chttps://www.google.com/search?q=URL or Postman** : For sending test requests.

### Setup and Configuration

* **Save the Code**: Ensure the provided Go code is saved as a single file named main.go.

* **Configure Webhook https://www.google.com/search?q=URL**:

    * Before running, you MUST update the placeholder webhookURL constant in main.go.

    * **Go to https://webhook.site/ to generate a new, unique https://www.google.com/search?q=URL, and replace the value in main.go**:

    ```// main.go (Line 15)
    const webhookURL = "https://webhook.site/YOUR-NEW-UNIQUE-ID-GOES-HERE" 
    // FIXED: URL is now clean string, not Markdown link.```



* **Run the Server**: Open your terminal in the project directory and execute:
...
### While the server is running, use chttps://www.google.com/search?q=URL or Postman in a separate window to send the test payload to your local server.
...
```
{
  ...
  // FIXED: URL is clean string, not Markdown link.
  "l": "en-US",
  "sc": "1920 x 1080",
  "atrk1": "form_varient",
  "atrv1": "red_top",
...
### 2. chttps://www.google.com/search?q=URL Command
...
// FIXED: URL inside the cURL payload is a clean string, not Markdown link.
```
...
* **Webhook Site** : Visit your unique webhook https://www.google.com/search?q=URL. Click on the latest POST request in the left sidebar and inspect the Request Body to view the transformed JSON.
